...
class MasterBlokCluster extends CI_Controller {
	...
	public function index(){
		...
		$data = array(
					...
					'data_blok_cluster' => $this->MainModel->dataCluster('no_urut','asc',80,0),
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer text-right">
        ...
        <a href="'.site_url("MasterBlokCluster/proses_hapus/".$rowdcl->kode_cluster).'" class="btn bg-gradient-danger btn-xs"><i class="fas fa-check"></i>&ensp;Ya !</a>
      </div>
    </div>
    <!-- /.modal-content -->
  </div>
...
<!-- /.modal -->';
			...
			if ($dt['u']!=0) {
        		$update = '
				<a href="'.site_url("MasterBlokCluster/sesi_edcl/".$rowdcl->kode_cluster).'" class="btn bg-gradient-primary btn-xs" onmouseover="$(this).tooltip(show)" onmouseout="$(this).tooltip(hide)" onblur="$(this).tooltip(hide)" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fas fa-edit"></i></a>';
        	} else {
        		$update = '';
        	}
	...
	public function sesi_edcl($idcl=null){
		if (!isset($idcl)||$idcl==null) {
			...
			redirect('MasterBlokCluster/edit_cluster/'.$idcl);
		}
	}
